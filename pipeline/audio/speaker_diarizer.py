...
def run_diarization(input_target: str, output_dir: str):
    print("▶️ [SpeakerDiarizer] Starting FULL PORTABLE Pyannote Mode...", flush=True)
    ...
    print(f"🎧 [SpeakerDiarizer] Analyzing mixed audio: {os.path.basename(source_audio)}", flush=True)
    ...
    print("🧠 [SpeakerDiarizer] AI is mapping the voices (CPU processing)...", flush=True)
    from pyannote.audio import Pipeline
    pipeline = Pipeline.from_pretrained(config_path)

    # 🛡️ THE FIX: Parse speaker config dynamically from UI
    min_speakers = None
    max_speakers = None
    ...
    if job_cfg_path and os.path.exists(job_cfg_path):
        with open(job_cfg_path, "r") as cfg:
            c = json.load(cfg)
            min_raw = c.get("min_speakers", "")
            max_raw = c.get("max_speakers", "")
